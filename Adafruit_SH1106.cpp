...
void Adafruit_SH1106::begin(uint8_t vccstate, uint8_t i2caddr, bool reset) {
  ...
  // turn on VCC (9V?)
    ...
    SH1106_command(SH1106_SEGREMAP | 0x1);  // 32 tall code, ignore this
...
void Adafruit_SH1106::turnOff()
{
    SH1106_command(SH1106_DISPLAYOFF);//--turn off oled panel
}
	...
	else
    {
    	for ( i = 0; i < height; i++)
        {
            ...
            for( j = 0; j < 8; j++)
            {
                ...
                if(_i2cHelper)
                {
                    _i2cHelper->SendBuffer(&buffer[p], width, 0x40);
                    p += width;
                }
#else
                Wire.beginTransmission(_i2caddr);
