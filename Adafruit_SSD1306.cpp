...
void Adafruit_SSD1306::begin(uint8_t vccstate, uint8_t i2caddr) {
  ...
  _i2cHelper = new I2CHelper(i2caddr);
#else
  // set pin directions
  if (sid != -1){
    pinMode(dc, OUTPUT);
  ...
  else
  {
   ...
   Wire.setClock(400000);  // this is the default nowadays. it was 400k orig
    ...
    ssd1306_command(SSD1306_SEGREMAP | 0x1);  				// orientation
    ssd1306_command(SSD1306_COMSCANDEC);
...
void Adafruit_SSD1306::turnOff()
{
    ssd1306_command(SSD1306_DISPLAYOFF);//--turn off oled panel
}
...
void Adafruit_SSD1306::ssd1306_data(uint8_t c) {
    ...
    if(_i2cHelper)
    {
        _i2cHelper->SendBuffer(&c, 1, 0x40);
    }
#else
  if (sid != -1)
  {
...
void Adafruit_SSD1306::display(void) {
  ...
  else
  {
    for (uint16_t i=0; i<(SSD1306_LCDWIDTH*SSD1306_LCDHEIGHT/8); i += 16)
    {
        ...
        if(_i2cHelper)
        {
            _i2cHelper->SendBuffer(&buffer[i], 16, 0x40);
        }
#else
